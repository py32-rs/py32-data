...
impl ChipInterrupts {
    ...
    pub(crate) fn process(
        &self,
        core: &mut py32_data_serde::chip::Core,
        chip_name: &str,
        ...
        core.interrupts = header_irqs
            .iter()
            .map(|(k, v)| py32_data_serde::chip::core::Interrupt {
                name: k.clone(),
                number: *v,
            })
            .collect();
        ...
        for p in &mut core.peripherals {
            if let Some(signals) = chip_signals.get(&p.name) {
                let mut all_irqs: Vec<py32_data_serde::chip::core::peripheral::Interrupt> = Vec::new();
                ...
                for (signal, irqs) in signals {
                    ...
                    for irq in irqs {
                        all_irqs.push(py32_data_serde::chip::core::peripheral::Interrupt {
                            signal: signal.clone(),
                            interrupt: irq,
                        })
                    }
