...
use py32_data_serde::chip::core::peripheral::Pin;
...
pub struct Chip {
    ...
    ram: u32,
    group_idx: usize,
    packages: Vec<py32_data_serde::chip::Package>,
}
...
fn merge_periph_pins_info(
    chip_name: &str,
    periph_name: &str,
    core_pins: &mut [py32_data_serde::chip::core::peripheral::Pin],
    af_pins: &[py32_data_serde::chip::core::peripheral::Pin],
) {
...
fn parse_group(
    ...
    for (package_i, package_name) in package_names.iter().enumerate() {
        ...
        chips
            ...
            .unwrap()
            .packages
            .push(py32_data_serde::chip::Package {
...
fn process_core(
    ...
    chip_af: Option<&HashMap<String, Vec<py32_data_serde::chip::core::peripheral::Pin>>>,
    dma_channels: &dma::DmaChannels,
) -> py32_data_serde::chip::Core {
                ...
                for periph in peri_kinds.keys() {
                    if let Some(signal) = signal.strip_prefix(&format!("{periph}_")) {
                        periph_pins.entry(periph.to_string()).or_default().push(
                            py32_data_serde::chip::core::peripheral::Pin {
                                pin: pin_name.clone(),
    ...
    for (pname, pkind) in peri_kinds {
        ...
        let mut p = py32_data_serde::chip::core::Peripheral {
            name: if pname == "SBS" {
                "SYSCFG".to_string()
            } else {
                pname.clone()
        ...
        if let Some(block) = peri_matcher.match_peri(&format!("{chip_name}:{pname}:{pkind}")) {
            p.registers = Some(py32_data_serde::chip::core::peripheral::Registers {
    ...
    if let Ok(extra_f) = std::fs::read(format!("data/extra/family/{}.yaml", group.family.as_ref().unwrap())) {
        ...
        struct Extra {
            peripherals: Vec<py32_data_serde::chip::core::Peripheral>,
        }
    ...
    let mut core = py32_data_serde::chip::Core {
...
fn process_chip(
    ...
    docs: &docs::Docs,
    group: &ChipGroup,
    cores: &[py32_data_serde::chip::Core],
) -> Result<(), anyhow::Error> {
            ...
            if let Some(banks) = banks {
                for bank in banks {
                    ...
                    for (index, address, size, settings) in regions {
                        ...
                        memory_regions.push(py32_data_serde::chip::Memory {
                            name,
                            kind: py32_data_serde::chip::memory::Kind::Flash,
                            address,
                            size,
    ...
    for each in [
        ...
        if let Some(address) = h.defines.get("all").unwrap().0.get(&format!("{each}_BASE")) {
            ...
            memory_regions.push(py32_data_serde::chip::Memory {
                name: key.to_string(),
                kind: py32_data_serde::chip::memory::Kind::Ram,
                address: u32::try_from(*address).unwrap(),
                size,
                settings: None,
            })
